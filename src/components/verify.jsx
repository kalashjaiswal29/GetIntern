...
const Verify = () => {
  ...
  const handleVerifySubmit = async (e) => {
    ...
    try {
      ...
      clearTimeout(timeoutId); // Request mil gayi toh timeout cancel karein
      setBackHome(false)
      setResult(data);
      setVerifyDone(true);
    } catch (error) {
  ...
  <div style={{ marginTop: "20px" }}>
    {result.success ? (
      ...
      /* --- FAILURE: Aapka Original Style (Red Text) --- */
      <div style={{ color: "red" }}>
        <p>❌ {result.message}</p>
        ...
        {/* --- FAILURE: Button Red text ke niche --- */}
        <div >
          <Link to="/">
            <button className={styles.homeBtn}>
              <span>← </span>Back To Home
